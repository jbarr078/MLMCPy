...
class MLMCSimulator(object):
    ...
    def get_model_inputs_to_run_for_each_level(self, sample_sizes,
        ...
        if cache_file_names is not None:
            inputs, cache_sample_sizes = \
                self._setup_modular_cache(inputs, cache_file_names)
    ...
    def _setup_modular_cache(self, inputs, cache_file_names):
        """
        Sets up some operations for modular MLMC cache.
        """
        updated_inputs, cache_sample_sizes, indices_to_delete = \
                self._compare_inputs_to_cache(inputs, cache_file_names[1])
    ...
    @staticmethod
    def _compare_inputs_to_cache(inputs, cache_input_file):
        """
        ...
        removed and a counter of the cached sample sizes that will be subtracted
        from the optimal sample sizes during get_model_inputs().
        """
        ...
        for i in range(len(cache_inputs)):
            indices_to_delete.append(np.argwhere(inputs ==
                                                 cache_inputs[i])[:, 0])

            cache_sample_sizes.append(indices_to_delete[i].size)

        new_inputs = np.delete(inputs, indices_to_delete)

        return new_inputs, cache_sample_sizes, indices_to_delete
    ...
    def _remove_unused_cached_outputs(cache_output_file, indices_to_save):
        ...
        np.savetxt(cache_output_file, updated_outputs)
    ...
    @staticmethod
    def _merge_cache_output(outputs, cache_outputs_file):
        """
        Takes in outputs and merges them with the cached outputs generated by
        compute_costs_and_variances().

        :return: Returns an array of merged cache and output data.
        :rtype: np.ndarray
        """
