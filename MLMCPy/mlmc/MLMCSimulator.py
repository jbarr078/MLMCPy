...
class MLMCSimulator(object):
    ...
    def get_model_inputs_to_run_for_each_level(self, sample_sizes,
        ...
        if cache_input_file is not None:
            inputs, cache_sample_sizes = \
                self._compare_inputs_to_cache(inputs, cache_input_file)
    ...
    def _compare_inputs_to_cache(inputs, cache_input_file):
        ...
        removed and a counter of the cached sample sizes.
        """
        ...
        for i in range(len(cache_inputs)):
            ...
            for j in range(len(cache_inputs[i])):
                if np.argwhere(inputs == cache_inputs[i][j]):
                    indices_to_delete.append(cache_inputs[i][j])
                    delete_counter += 1

            cache_sample_sizes.append(delete_counter)

        new_inputs = np.delete(inputs, indices_to_delete)

        return new_inputs, cache_sample_sizes
