...
class MLMCSimulator(object):
    ...
    def compute_costs_and_variances(self, user_sample_size=None,
        ...
        if cache:
            HDF5Storage.write_cache_to_hdf5(cache, self._cached_inputs,
    ...
    def get_model_inputs_to_run_for_each_level(self, sample_sizes, cache=None):
        """
        ...
        if cache:
            inputs, cache_sample_sizes = \
                self._modify_modular_cache(sample_sizes, inputs, cache)
    ...
    @staticmethod
    def load_model_outputs_for_each_level(filenames=None, cache=None):
        """
        ...
        if filenames is not None:
            if isinstance(filenames, list):
                for level, filename in enumerate(filenames):
                    ...
                    if cache:
                        outputs = \
                            HDF5Storage.merge_cache_output(outputs, level,
                                                           cache)
