...
class MLMCSimulator(object):
    ...
    def compute_costs_and_variances(self, user_sample_size=None,
        ...
        if cache_file_names:
            self._write_cache_to_file(cache_file_names, self._cached_outputs,
                                      self._cached_inputs)
    ...
    def _write_cache_to_file(cache, cache_outputs, cache_inputs):
        """
        Writes the outputs and inputs generated by compute_costs_and_variances()
        to files. Users can define file names by using a list with a file name
        for outputs and inputs, otherwise they will be given default names.

        :param cache: Optional user defined file names as a list,
            otherwise a bool.
        """
        # Note - original shape is (models, -1, 1), need to reshape to 2D (TODO)
        if isinstance(cache, list):
            np.savetxt(cache[0], cache_outputs.reshape(len(cache_outputs), -1))
            np.savetxt(cache[1], cache_inputs.reshape(len(cache_inputs), -1))
        else:
    ...
    @staticmethod
    def _compare_inputs_to_cache(sample_sizes, inputs, cache_input_file):
        """
        Takes a distribution of inputs generated by draw_sample() and compares
        them to the cache. Returns a np.ndarray of inputs with the cached_inputs
        removed, cached_sample_sizes that will be removed from sample sizes,
        and indices to save for the cached outputs file.
        """
        cache_inputs = \
            np.genfromtxt(cache_input_file).reshape(len(sample_sizes), -1, 1)
        ...
        return new_inputs, cache_sample_sizes, indicies

    @staticmethod
    def _remove_unused_cached_outputs(cache_output_file, indices_to_save,
                                      merged_cache_files=None):
        """
        Reads in the cache_output_file and uses the indices determined by the
        _compare_inputs_to_cache() method to save the corresponding values. Then
        updates the cache_output_file with these values.
        """
        ...
        if merged_cache_files is not None:
            for i in range(len(updated_outputs)):
                np.savetxt(merged_cache_files[i], updated_outputs[i])
        else:
            for i in range(len(updated_outputs)):
    ...
    @staticmethod
    def load_model_outputs_for_each_level(filenames=None, cache_enabled=False,
                                          merged_cache=None):
        """
        ...
        if filenames is not None:
            if isinstance(filenames, list):
                for level, filename in enumerate(filenames):
                    ...
                    if cache_enabled:
                        outputs = \
                            MLMCSimulator._merge_cache_output(outputs, level,
                                                              merged_cache)
    ...
    @staticmethod
    def _merge_cache_output(outputs, level, merged_cache=None):
        """
        ...
        if merged_cache is not None:
            cache_outputs_file = merged_cache[level]
        else:
            cache_outputs_file = 'level%s_cache.txt' % level

        with np.warnings.catch_warnings():
            np.warnings.simplefilter('ignore')
            cache_outputs = np.genfromtxt(cache_outputs_file)
